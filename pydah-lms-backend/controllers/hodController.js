...
const getDepartmentEmployees = async (req, res) => {
  try {
    ...
    const employees = await Employee.find({
      ...
      status: 'active'
    }).select('name email employeeId department phoneNumber leaveBalance status designation');
...
const getDashboard = async (req, res) => {
  try {
    ...
    .select('name email employeeId department phoneNumber leaveBalance status designation leaveRequests')
    .populate({
      path: 'leaveRequests.alternateSchedule.periods.substituteFaculty',
      select: 'name'
    });
...
const updateEmployeeDetails = async (req, res) => {
  try {
    ...
    const allowedUpdates = ['name', 'email', 'phoneNumber', 'designation', 'status'];
