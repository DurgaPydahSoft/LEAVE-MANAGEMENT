...
exports.getCampusEmployees = async (req, res) => {
  try {
    ...
    const employees = await Employee.find(query)
      .select('name email employeeId department status phoneNumber designation role branchCode leaveBalance')
      .sort({ name: 1 });
...
exports.updateEmployee = async (req, res) => {
  try {
    ...
    const allowedUpdates = ['name', 'email', 'phoneNumber', 'role', 'status', 'leaveBalance'];
